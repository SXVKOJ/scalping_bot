...
async def get_actual_order_status_async(user: User, symbol: str, order_id: str) -> str:
    """Async function to get order status using MexcRestClient."""
    try:
        client = MexcRestClient(user.api_key, user.api_secret)
        response = await client.query_order(symbol, {"orderId": order_id})
        if not isinstance(response, dict):
            logger.error(
                f"⚠️ Ответ не является dict: {type(response)} — {response}",
                extra={"user_id": getattr(user, "telegram_id", None)},
            )
            return None
        return response.get("status", "UNKNOWN")
    except Exception as e:
        logger.exception(
            f"Ошибка при получении статуса ордера {order_id}: {e}",
            extra={"user_id": getattr(user, "telegram_id", None)},
        )
        return None
...
def check_mexc_keys(api_key: str, api_secret: str) -> tuple:
    ...
    response = requests.get(
        f"{url}?{query_string}&signature={signature}", headers=headers
    )
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                logger.info(
                    f"Response status code: {status_code}, response text: {response_text}"
                )
                ...
                if status_code != 200:
                    ...
                    if (
                        "700006" in response_text
                        and "ip white list" in response_text.lower()
                    ):
...
def get_user_client(telegram_id: int):
    try:
        ...
        return Spot(api_key=user.api_key, api_secret=user.api_secret), user.pair
    except User.DoesNotExist:
        raise ValueError("Пользователь не найден")
    except Exception as e:
        logger.error(
            f"Ошибка при получении клиента для пользователя {telegram_id}: {e}"
        )
