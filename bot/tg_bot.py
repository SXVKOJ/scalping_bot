...
if not config_obj.bot_token:
    raise SystemExit("TELEGRAM_TOKEN is not set. Copy .env.example to .env and fill the token.")
if not config_obj.pair:
    raise SystemExit("PAIR is not set. Example: PAIR=BTC/USDT in .env")


def exception_handler(loop, context):
    """Global exception handler to catch uncaught exceptions and prevent bot death."""
    msg = context.get("exception", context["message"])
    logger.critical(f"Uncaught exception in event loop: {msg}", exc_info=True)
    # Don't stop the loop - allow it to continue


async def main():
    try:
        logger.info("Starting bot initialization...")
        logger.info(f"Trading pair for this instance: {config_obj.pair}")
        proxy = get_telegram_proxy()
        if proxy:
            logger.info(f"Telegram proxy enabled: {proxy.split('@')[-1]}")
        bot = create_bot(config_obj.bot_token)
        # Сохраняем экземпляр бота в глобальную переменную для доступа из других модулей
        config.bot_instance = bot

        # Устанавливаем глобальный обработчик исключений для event loop
        ...
        websocket_init_task = asyncio.create_task(
            websocket_manager.connect_valid_users()
        )
