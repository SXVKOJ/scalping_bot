...
| Переменная | По умолчанию | Смысл |
|---|---|---|
| `ENTRY_ORDER_TYPE` | `market` | `market` — как раньше, берёт любую цену из стакана. `ioc` — лимит по ask с запасом `ENTRY_MAX_SLIPPAGE_PCT`, остаток отменяется: худшая цена входа ограничена, но часть сигналов останется без входа. |
| `ENTRY_MAX_SLIPPAGE_PCT` | `0.3` | Допустимое проскальзывание для `ioc`-входа, %. |
| `PROFIT_FEE_BUFFER_PCT` | `0` | Надбавка к профиту на комиссии, %. У MEXC спот taker 0.05%, maker 0% — поставьте `0.05`, чтобы профит из `/parameters` считался «чистым». |
| `MARKET_SILENCE_TIMEOUT_SEC` | `90` | Через сколько секунд тишины в потоке цен переподключаться. |

Точность цены и количества бот берёт из `GET /api/v3/exchangeInfo` и кеширует
на 12 часов. Если биржа недоступна, используется прежнее поведение
(6 знаков в цене), а сделка всё равно защищается лимитом на продажу.
